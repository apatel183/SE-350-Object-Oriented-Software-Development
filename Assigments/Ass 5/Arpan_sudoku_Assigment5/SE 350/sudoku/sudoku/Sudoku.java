...
public class Sudoku {
    ...
    private void checkRep() {
    	...
    	for (int i = 0; i < size; i++){
    		for (int j = 0; j <size; j++){
    			assert this.square[i][j] >= -1: "Sudoku, Rep invariant: greater or equal";
    			assert this.square[i][j] < size: "Sudoku, Rep invariant: value less than size";
    		}
    	}
    ...
    public Sudoku(int dim) {
    	...
    	checkRep();// throw new RuntimeException("not yet implemented.");
    }
    ...
    public static Sudoku fromFile(int dim, String filename) throws IOException,
    	...
    	try {
    		while ((l_text = r.readLine()) != null){
    			if (total >=  size){
    				throw new ParseException("incorrect formate of rows");
    			}
    			int [] nRow = new int[size];
    			char [] a = l_text.toCharArray();
    			int l = l_text.length();
    			if (l != (dim*dim)){
    				throw new ParseException("incorrct formate of columns");
    			}
    			for (int i = 0; i <size; i++){
    				if (a[i] == '.')
    					nRow[i]=0;
    ...
    public Sudoku interpretSolution(Environment e) {
    	...
    	for (int i = 0; i <size;i++){
    		for (int j = 0; j <size;j++){
    			for (int k = 0; k <size; k++){
    				Bool v = e.get(occupies[i][j][k]);
    				if(v.equals(Bool.TRUE)){
